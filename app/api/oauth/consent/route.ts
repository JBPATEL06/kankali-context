...
export async function POST(req: Request) {
  ...
  const resource = body.resource || mcpGitResource(req);
  ...
  const registered = client.redirect_uris.some(
    (u) => u === redirectUri || u.replace(/\/+$/, "") === redirectUri.replace(/\/+$/, "")
  );
  if (!registered || !isAllowedRedirectUri(redirectUri)) {
    return Response.json(
      {
        error: "invalid_request",
        error_description: `redirect_uri not allowed: ${redirectUri}`,
      },
      { status: 400 }
    );
  }
