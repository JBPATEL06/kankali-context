/**
 * Kankali Drive MCP — free-form file/folder CRUD & SDLC context management on the user's Google Drive App Data folder.
 * Endpoint: https://kankali-context.vercel.app/mcp/Drive
 *
...
import { z } from "zod";
import { getUser, getUserByMcpKey } from "@/lib/users";
...
function getRequestContext(): RequestContext {
  return requestContext.getStore() ?? { user: null, accessToken: null, authError: null };
}
...
function unauthorizedResponse(message: string, req?: Request): Response {
  ...
  const meta = `${origin}/.well-known/oauth-protected-resource/mcp/Drive`;
  const resource = `${origin}/mcp/Drive`;
  return new Response(
    JSON.stringify({
      error: "unauthorized",
...
async function resolveCaller(req: Request): Promise<{
  ...
  if (!user.googleRefreshTokenEnc) {
    throw new Error(
      "Google Drive App Data not connected. Open https://kankali-context.vercel.app/settings and Connect Google Drive."
    );
  }

  let refreshToken: string;
  try {
    refreshToken = decrypt(user.googleRefreshTokenEnc);
  } catch {
    throw new Error("Failed to decrypt Google Drive token. Please reconnect Google Drive.");
  }
...
function withUser<T extends z.ZodTypeAny>(
  ...
  return async (args: z.infer<T>) => {
    try {
      const { user, accessToken, authError } = getRequestContext();
      if (!accessToken || !user) {
        const detail =
          authError ||
      ...
      return await fn(accessToken, args, user);
    } catch (err: unknown) {
...
const handler = createMcpHandler((server) => {
  server.registerTool(
    ...
    {
      description:
        "Read NOTICE.md (how to use this Google Drive context vault). MUST be called first every session. Bootstraps NOTICE.md + index.md if missing.",
      inputSchema: readNoticeSchema.shape,
    },
    withUser(readNoticeSchema, async (accessToken) => toolReadNotice(accessToken))
  );
  ...
  server.registerTool(
    ...
    withUser(readIndexSchema, async (accessToken) => toolReadIndex(accessToken))
  );
  ...
  server.registerTool(
    ...
    withUser(readOutlineSchema, async (accessToken, args) => toolReadOutline(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(listTreeSchema, async (accessToken, args) => toolListTree(accessToken, args))
  );

  server.registerTool(
    "read_file",
    {
      description: `${MUST} Read any file by path relative to drive root.`,
      inputSchema: readFileSchema.shape,
    },
    withUser(readFileSchema, async (accessToken, args) => toolReadFile(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(writeFileSchema, async (accessToken, args) => toolWriteFile(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(deletePathSchema, async (accessToken, args, user) => toolDeletePath(accessToken, args, user.uid))
  );

  server.registerTool(
    "search_files",
    {
      description: `${MUST} Keyword search across files in the drive vault.`,
      inputSchema: searchFilesSchema.shape,
    },
    withUser(searchFilesSchema, async (accessToken, args) => toolSearchFiles(accessToken, args))
  );

  server.registerTool(
    "project_upsert",
    {
      description: `${MUST} Create/update project/<slug> status.md, docs/ (overview, plan, audit), codebase/, and resources/.`,
      inputSchema: projectUpsertSchema.shape,
    },
    withUser(projectUpsertSchema, async (accessToken, args) => toolProjectUpsert(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(projectCodebaseNoteSchema, async (accessToken, args) => toolProjectCodebaseNote(accessToken, args))
  );

  server.registerTool(
    "project_get",
    {
      description: `${MUST} Read project folder. Supports section parameter: status | docs | codebase | resources | all.`,
      inputSchema: projectGetSchema.shape,
    },
    withUser(projectGetSchema, async (accessToken, args) => toolProjectGet(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(projectListSchema, async (accessToken) => toolProjectList(accessToken))
  );
  ...
  server.registerTool(
    ...
    withUser(currentSessionSetSchema, async (accessToken, args) => toolCurrentSessionSet(accessToken, args))
  );
  ...
  server.registerTool(
    ...
    withUser(currentSessionGetSchema, async (accessToken) => toolCurrentSessionGet(accessToken))
  );
});
...
async function wrappedHandler(req: Request): Promise<Response> {
  ...
  return requestContext.run({ user, accessToken, authError }, async () => {
